...
| [02](https://github.com/canada-ca/cloud-guardrails/blob/master/EN/02_Management-Admin-Privileges.md),[03](https://github.com/canada-ca/cloud-guardrails/blob/master/EN/02_Management-Admin-Privileges.md)| If AWS SSO or other identity provider is being used by the end department’s AWS Organizations account: Verify it is deployed in Canada. Verify that either AWS SSO, AWS Managed Active Directory or the identity provider is configured to require MFA. |  |
...
| [04](https://github.com/canada-ca/cloud-guardrails/blob/master/EN/04_Enterprise-Monitoring-Accounts.md)|Verify that the “BillingAndAccountAccess”” policy exists. |  |
...
| [11](https://github.com/canada-ca/cloud-guardrails/blob/master/EN/11_Logging-and-Monitoring.md)| Logging must be enabled for the cloud environment and for cloud-based workloads. Navigate the end department’s accounts’ Trusted Advisor “AWS CloudTrail Logging” parameter to ensure that centralized logging to an S3 bucket is enabled for all regions within each individual account. This also includes the master account which is immune to Service Control Policies (SCPs). Navigate to Cloudtrail, select the active trail, and validate that both S3 (would not be applicable for the department’s Logging account) and Lambda Data Events are enabled for “all S3 buckets in your account “ and “all current and future functions”. Validate that all end department’s VPCs have VPC Flow Logs enabled for all requests with logging sent to a centralized S3 bucket within the AWS Landing Zone’s Logging account. Validate that the AWS Organization’s core and non-core, OU-based PBMM accounts are subject to an SCP that denies any modification to the AWS CloudTrail, VPC Flow Logs and CloudWatch Logging configurations, but exclude the Landing Zone Execution role. | |
